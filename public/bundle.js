...
// pres chart
{
  ...
  var timeFrame = [1732, 1790];
  var svg = d3.select('#presidents svg');

  svg.attr('width', window.innerWidth);
  svg.attr('height', window.innerHeight);

  var margin = { top: 20, right: 0, bottom: 150, left: 300 },
      width = +svg.attr('width') - margin.left - margin.right,
      height = +svg.attr('height') - margin.top - margin.bottom;

  var g = svg.append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');

  d3.json('./data/pres.json', function (err, data) {
    if (err) throw err;

    data.reverse();

    data.map(function (pres) {
    ...
    var updateChart = function updateChart(newData) {
      ...
      {
        var yearsAliveBars = g.selectAll('.years-alive').data(newData);

        yearsAliveBars.exit().transition(t).attr('height', 0).remove();

        var grover = false;

        yearsAliveBars //.transition(t)
        .attr('x', function (d) {
          return x(d.yearsAlive[0]);
        ...
        yearsAliveBars.enter().append('rect').attr('class', 'bar years-alive').attr('x', function (d) {
          return x(d.yearsAlive[0]);
      ...
      {
        var yearsPresBars = g.selectAll('.years-pres').data(newData);

        yearsPresBars.exit().remove();

        yearsPresBars //.transition(t)
        .attr('x', function (d) {
          return x(d.yearsAsPres[0]);
        }).attr('width', function (d) {
          return x(d.numYearsAsPres + min);
        }).transition(t).attr('y', function (d) {
          return y(d.president);
        }).attr('height', y.bandwidth());

        yearsPresBars.enter().append('rect').attr('class', 'bar years-pres').attr('x', function (d) {
          return x(d.yearsAsPres[0]);
        }).attr('height', y.bandwidth()).attr('y', function (d) {
          return y(d.president);
        }).attr('width', function (d) {
      ...
      {
        // NAMES
        var presNames = g.selectAll('.pres-names').data(newData);
        ...
        presNames.attr('x', function (d) {
          return x(d.yearsAlive[0]);
        }).attr('width', function (d) {
          return x(d.numYearsAlive + min);
        }).attr('height', y.bandwidth()).text(function (d) {
          return d.president;
        }).transition(t).attr('y', function (d) {
          return y(d.president) + 10;
        });

        presNames.enter().append('text').merge(presNames).attr('class', 'pres-names').attr('x', function (d) {
          return x(d.yearsAlive[0]);
        }).attr('height', y.bandwidth()).text(function (d) {
          return d.president;
        }).transition(t).attr('y', function (d) {
    ...
    var narrowTimeFrame = function narrowTimeFrame(start, end) {
      var clone = data.clone();
      clone = clone.map(function (pres) {
      ...
      clone = clone.filter(function (pres) {
